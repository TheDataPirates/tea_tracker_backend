...
exports.getAllLots = async (req, res, next) => {
    ...
    try {
        ...
        for (let bulk_id of todayBulks) {
            ...
            for (let lot_ele of allLots){
                todayLots.push(lot_ele.dataValues);

            }
        }
        console.log(todayLots);
...
exports.getRemeasureLots = async (req, res, next) => {
    let todayLots = [];
    try {
        const todayBulks = await Bulk.findAll({
            attributes: ['bulk_id'],
            where: {method: {[Op.like]: 'Remeasuring'}, date: new Date()}, // This should be today's date
        });
        for (let bulk_id of todayBulks) {
            let allLots = await Lot.findAll({
                attributes: ['lot_id', 'grade_GL', 'gross_weight', 'no_of_container', 'water', 'course_leaf', 'other', 'net_weight', 'deduction', 'container_type', 'BulkBulkId', 'BoxBoxId'],
